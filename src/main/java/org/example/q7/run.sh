...
# Script chạy MapReduce Question7 với test case cụ thể
...
TEST_CASE=$1
INPUT_DIR="/user/input/question7_${TEST_CASE}"
OUTPUT_DIR="/user/output/question7_${TEST_CASE}"
...
echo "=== Chạy Image Histogram cho ${TEST_CASE} ==="
...
hadoop jar question7.jar Question7 $INPUT_DIR $OUTPUT_DIR

if [ $? -eq 0 ]; then
    echo ""
    echo "=== Kết quả Image Histogram cho ${TEST_CASE} ==="
